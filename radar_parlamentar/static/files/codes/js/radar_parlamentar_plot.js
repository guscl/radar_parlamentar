...
// Load the data.
d3.json("dados.json", function(partidos) {
  ...
  // Add a title.
  dot.append("title")
      .text(function(d) { return nome_partido; });
  ...
  function position(dot) {
    dot .attr("cx", function(d) { return xScale(x(d)); })
        ...
        .attr("r", function(d) { return radiusScale(tamanhoPartido(d)); });
  }

  // Defines a sort order so that the smallest dots are drawn on top.
  function order(a, b) {
    return tamanhoPartido(b) - tamanhopartido(a);
  }
  ...
  function interpolateData(year) {
    return partidos.map(function(d) {
      return {
        nome: d.nome_partido,
        cor: d.cor,
        tamanhoPartido: interpolateValues(d.tamanhoPartido, year),
